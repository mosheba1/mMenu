...
RMenu.Add('combat', 'main', RageUI.CreateMenu("~w~Server Name","",0,100)) ------ change this to whatever you want
RMenu:Get('combat', 'main'):SetSubtitle("Your subtitle here")------ change this to whatever you want
RMenu.Add('combat', 'weapons', RageUI.CreateSubMenu(RMenu:Get('combat','main'), "Weapon Menu", "Choose your weapon", nil, nil))
RMenu.Add('combat', 'vehicles', RageUI.CreateSubMenu(RMenu:Get('combat','main'), "Vehicle Menu", "Select a vehicle", nil, nil))
RMenu.Add('combat', 'teleport', RageUI.CreateSubMenu(RMenu:Get('combat','main'), "Teleport Menu", "Select a location to teleport to", nil, nil))

RageUI.CreateWhile(1.0, RMenu:Get('combat', 'main'), 311, function()
    RageUI.IsVisible(RMenu:Get('combat', 'main'), true, true, true, function()
        if not inZone then
            RageUI.Button("~b~Apply Health", "~g~Select to add Health", { }, true, function(Hovered, Active, Selected)
                if (Selected) then
                    if timers.health <= 0 then
                        timers.health = cooldowns.health
                        SetEntityHealth(PlayerPedId(), 200)
                        notify("~g~Refilled Health")
                    else
            ...
            RageUI.Button("~b~Apply Armour", "~g~Select to add Armor", { }, true, function(Hovered, Active, Selected)
                if (Selected) then
                    if timers.armour <= 0 then
                        timers.armour = cooldowns.armour
                        SetPedArmour(PlayerPedId(), 100)
                        Notify("~g~Refilled Armor")
                    else
            ...
            if Config.WeaponMenu == true then
                RageUI.Button("~b~Weapon Menu", "Select to open the weapon spawner", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                    if (Selected) then end
                end, RMenu:Get('combat','weapons'))
            end

            if Config.VehicleSpawnMenu == true then
                RageUI.Button("~b~Vehicle Menu", "Select to open Vehicle Menu", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                    if (Selected) then end
            ...
            RageUI.Button("~b~Clear Loadout", "Select to clear your loadout", { }, true, function(Hovered, Active, Selected)
                if (Selected) then
